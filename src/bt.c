...
unsigned char lo(unsigned int val) {
    return (unsigned char) (val & 0xff);
}

unsigned char hi(unsigned int val) {
    return (unsigned char) (val & 0xff00) >> 8;
}

unsigned int bt_isReadByteReady() {
    return (!(mmio_read(ARM_UART0_FR) & 0x10));
}
...
int hciCommandBytes(unsigned char *opcodebytes, volatile unsigned char *data, unsigned char length) {
    ...
    if (code == CONNECT_COMPLETE_CODE) {
        ...
        if (err != 0) {
            printf("BT: Saw HCI COMMAND STATUS error err \n");
            return 12;
        }
...
void bt_reset() {
    volatile unsigned char emtpy[] = {};

    if (hciCommand(OGF_HOST_CONTROL, COMMAND_RESET_CHIP, emtpy, 0)) {
        printf("BT: Reset failed \n");
    }
}

void bt_loadfirmware() {
    volatile unsigned char emtpy[] = {};
    if (hciCommand(OGF_VENDOR, COMMAND_LOAD_FIRMWARE, emtpy, 0)) {
        printf("BT: Firmware Load failed \n");
    }
    ...
    while (c < size) {
        ...
        if (hciCommandBytes(opcodebytes, data, length)) {
            printf("BT: Firmware data load failed. \n");
            break;
        }
...
void bt_setbaudaddr() {
    ...
    if (hciCommand(OGF_VENDOR, COMMAND_SET_BDADDR, command, 6)) {
        printf("BT: Setting baud address failed. \n");
    }
}
...
void setLEeventmask(unsigned char mask) {
    ...
    if (hciCommand(OGF_LE_CONTROL, 0x01, command, 8)) {
        printf("BT: Setting LE event mask failed.\n");
    }
}
...
void setLEscanenable(unsigned char state, unsigned char duplicates) {
    ...
    if (hciCommand(OGF_LE_CONTROL, 0x0c, command, 2)) {
        printf("BT: Setting LE scaneable failed.\n");
    }
}

void setLEscanparameters(unsigned char type, unsigned char linterval, unsigned char hinterval, unsigned char lwindow,
                         unsigned char hwindow, unsigned char own_address_type, unsigned char filter_policy) {
    ...
    if (hciCommand(OGF_LE_CONTROL, 0x0b, command, 7)) {
        printf("BT: Setting LE scan parameters failed.\n");
    }
}
...
void setLEadvertenable(unsigned char state) {
    ...
    if (hciCommand(OGF_LE_CONTROL, 0x0a, command, 1)) {
        printf("Setting LE advertable failed.\n");
    }
}

void setLEadvertparameters(unsigned char type, unsigned char linterval_min, unsigned char hinterval_min,
                           unsigned char linterval_max, unsigned char hinterval_max, unsigned char own_address_type,
    ...
    if (hciCommand(OGF_LE_CONTROL, 0x06, command, 15)) {
        printf("BT: Setting LE advert paramteres failed.\n");
    }
}
...
void setLEadvertdata() {
    ...
    if (hciCommand(OGF_LE_CONTROL, 0x08, command, 32)) {
        printf("BT: Setting LE advert data failed.\n");
    }
}
...
void startActiveAdvertising() {
    ...
    setLEadvertparameters(LL_ADV_NONCONN_IND, lo(min_interval), hi(min_interval), lo(max_interval), hi(max_interval), 0,
                          0);
    setLEadvertdata();
    setLEadvertenable(1);
}
