...
void scrollUp() {
    for (int i = 0; i < SCREEN_WIDTH / FONT_WIDTH; i++) {
        for (int j = 0; j < SCREEN_HEIGHT / FONT_HEIGHT - 1; j++) {
            screen[i][j] = screen[i][j + 1];
        }
        screen[i][SCREEN_HEIGHT / FONT_HEIGHT - 1] = 0;
    }
}

void drawCharToBuffer(unsigned char ch, int x, int y) {
    // Char within screen bounds
    if (x >= 0 && x < SCREEN_WIDTH / FONT_WIDTH && y >= 0 && y < SCREEN_HEIGHT / FONT_HEIGHT) {
        screen[x][y] = ch;
    }
}
...
void printf(const char *str) {
    while (*str) {
        ...
        if (cursorX >= SCREEN_WIDTH / FONT_WIDTH) {
            cursorX = 0;
            cursorY++;

            if (cursorY >= SCREEN_HEIGHT / FONT_HEIGHT) {
                scrollUp();
                cursorY = SCREEN_HEIGHT / FONT_HEIGHT - 1;
            }
        }
